...
INSERT INTO tasks(id, title, description, image_url) VALUES
	((SELECT nextval('tasks_seq') + 1) + 1, 'Clean Robot Sensors', 'Use the provided cloth to wipe sensors clean.', 'https://picsum.photos/300/300');
...
INSERT INTO user_tasks(id, user_id, task_id, created_time, is_skipped, is_complete) VALUES
	((SELECT nextval('user_tasks_seq') + 1), (select max(id) from users), (select max(id) from tasks), current_timestamp, false, false);
